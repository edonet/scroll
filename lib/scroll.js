...
export default class Scroll extends EventEmitter {
    ...
    reset(dx, dy, duration = true) {
        ...
        if (Math.abs(dx) >= 1 || Math.abs(dy) >= 1) {
            if (duration === true) {
                duration = Math.min(350, Math.max(Math.abs(dx), Math.abs(dy)) * 20);
            }
    ...
    translateBy(dx = 0, dy = 0) {
        ...
        // 判断是否需要滚动
        if (Math.abs(dx) >= 1 || Math.abs(dy) >= 1) {
            return this.$$touch.dispatch('translateBy', { dx, dy });
        }
    ...
    scrollBy(dx, dy, duration, easing = 'linear') {
        if (dx || dy) {
            ...
            if (duration > 50 && (Math.abs(dx) > 5 || Math.abs(dy) > 5)) {
                return this.$$touch.dispatch('scrollBy', { dx, dy, duration, easing });
            }
    ...
    destroy() {
        ...
        // 取消事件监听
        this.$$touch = this.$$touch.destroy() && null;
